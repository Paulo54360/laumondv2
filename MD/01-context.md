...
- Structure : pages Nuxt dans `pages/`; composants spécifiques dans `components/[feature]/`; logic métier dans `composables/` (useGallery, useNavbar...) ; styles globaux dans `assets/css/main.scss`, par page dans `assets/css/pages/` et par composant dans `assets/css/components/`. Assets publics dans `public/`.
