<template>
  <div class="gallery-container">
    ...
    <div class="pagination-controls">
      ...
      <select v-model="itemsPerPage" @change="currentPage = 0" id="itemsPerPage">
      ...
      <button @click="nextPage" :disabled="endIndex >= totalItems">Suivant &#8594;</button>
    </div>

    <div class="image-gallery">
      <div v-for="(url, index) in paginatedImages" :key="url" class="image-item" @click="openModal(index)">
    ...
    <div v-if="currentImageIndex !== null" class="modal" @click="closeModal">
      <img :src="imageUrls[currentImageIndex]" :alt="titles[imageUrls[currentImageIndex]]" class="modal-image" />
    </div>
  </div>
</template>
...
const endIndex = computed(() => Math.min(startIndex.value + itemsPerPage.value, imageUrls.value.length));
