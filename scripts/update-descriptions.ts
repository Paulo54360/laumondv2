...
// Structure des catégories et leurs titres
...
async function updateDescriptions() {
  ...
  for (const [category, works] of Object.entries(categoryTitles)) {
    console.log(`\nMise à jour de la catégorie: ${category}`);
    const categoryPath = join(baseDir, category);

    // Vérifier si le dossier de la catégorie existe
    try {
      await fs.access(categoryPath);
    } catch {
      console.log(`! Le dossier ${category} n'existe pas, création...`);
      await fs.mkdir(categoryPath, { recursive: true });
    }

    // Pour chaque dossier dans la catégorie
    for (const [folder, content] of Object.entries(works)) {
      const folderPath = join(categoryPath, folder);
      const mainTxtFile = join(folderPath, 'description.txt');

      try {
        // Créer le dossier principal et son fichier description.txt
        await fs.mkdir(folderPath, { recursive: true });
        await fs.writeFile(mainTxtFile, content.title);
        console.log(`✓ Mis à jour: ${category}/${folder}/description.txt`);
        updatedCount++;

        // Pour chaque image, créer son sous-dossier et son fichier description.txt
        for (const [imageNum, description] of Object.entries(content.images)) {
          const imagePath = join(folderPath, imageNum);
          const imageTxtFile = join(imagePath, 'description.txt');

          await fs.mkdir(imagePath, { recursive: true });
          await fs.writeFile(imageTxtFile, description);
          console.log(`  ✓ Mis à jour: ${category}/${folder}/${imageNum}/description.txt`);
          updatedCount++;
        }
      } catch (error) {
